...
内部主题链接会按 `base` 解析；外部 HTTP(S) 链接会带安全的关系属性。960px 以下，固定侧栏变成带标签的模态 Sheet；桌面宽度下，它持续显示在正文旁。
...
locale 条目定义 BCP 47 `lang`、显示标签与路由 `root`，并可覆盖导航、侧栏、首页与界面消息。路由镜像后，语言切换会保留当前页面。个别页面仍可用 frontmatter `lang` 覆盖。
