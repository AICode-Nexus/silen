...
内部主题链接会按 `base` 解析。HTTP(S) 导航链接会在当前上下文中按浏览器普通导航处理。nav 条目只传递 `href`，不会自动增加新上下文或关系属性。确实需要新上下文时，MDX 正文链接可显式设置 `target="_blank"` 与 `rel="noopener noreferrer"`。960px 以下，固定侧栏变成带标签的模态 Sheet；桌面宽度下，它持续显示在正文旁。
