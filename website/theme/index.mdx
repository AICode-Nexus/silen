...
Internal theme links are resolved under `base`; external HTTP(S) links open in
a new context with safe relationship attributes. Below 960px the persistent
sidebar becomes a labelled modal sheet. At desktop widths, it remains visible
beside the document.
...
switch preserve the current page. A page-level `lang` frontmatter value remains
available for exceptional content.
