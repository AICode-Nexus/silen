...
Before uploading, run the deterministic quality chain:
...
Exit codes `0`, `1`, and `2` from `ai eval` mean pass, retrieval failure, and
setup failure. The optional `.silen/ai/index.json` workspace snapshot can be
missing or stale without blocking audit; MCP search remains in memory.

Then preview a nested route directly, open a missing route, and inspect `llms.txt` plus
