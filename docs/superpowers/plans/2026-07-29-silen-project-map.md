# Silen Project Map Implementation Plan
...
**Goal:** Add one evidence-backed Silen project map and make it the default
planning entry point for maintainers and repository agents.

**Architecture:** `docs/project-map.md` is the only roadmap source of truth.
The root `AGENTS.md` supplies stable execution precedence without copying map
items, and `README.md` supplies one human-facing link. Version 1 remains plain
Markdown and uses read-only verification commands instead of adding a roadmap
parser or generator.

**Tech Stack:** Markdown, Git, Node.js 20.19 or newer for read-only validation,
and Prettier 3.9.5.

## Global Constraints

- Explicit user instructions always override the map.
- Preserve the existing Core, Default Theme, AI, Plugin, and Quality/Release
  planning boundaries and phase names.
- `docs/project-map.md` is the sole source of product position, work-item state,
  and default execution order.
- `AGENTS.md` and `README.md` may link to the map but must not duplicate its
  priority or status tables.
- At most one map-selected item may be `Active`.
- `Candidate` and `Watch` items are never default-executable.
- Old unchecked implementation-plan boxes are historical scaffolding, not
  current product evidence.
- Every shipped claim must link to code, tests, a commit, a release, or current
  documentation.
- The map decides prioritization; it does not authorize publication,
  deployment, destructive actions, or other external-state changes.
- Do not add a roadmap schema, parser, generator, dashboard, dependency, or
  runtime behavior in this implementation.
- Use repository-relative links for repository evidence and official HTTPS
  links for volatile external sources.
- The canonical map is English-only so there is no second translated status
...
- Create `docs/project-map.md`: canonical product baseline, ordered work
  horizons, lifecycle, evidence, intake, and maintenance rules.
- Create `AGENTS.md`: stable repository-level routing and authority rules.
- Modify `README.md`: one discoverability link near contributing guidance.
- Reference
  `docs/superpowers/specs/2026-07-29-silen-project-map-design.md`: the approved
  requirements; no implementation content belongs here.

### Task 1: Create the canonical evidence-backed project map
...
- Reference: `docs/superpowers/specs/*.md`
- Reference: `docs/superpowers/plans/*.md`
- Reference: `website/.silen/ai-evals.json`

**Interfaces:**

- Consumes: the approved project-map design, repository release evidence, and
  official dated ecosystem sources.
- Produces: the canonical `docs/project-map.md` path; stable IDs `CORE-001`,
  `THEME-001`, `AI-001` through `AI-009`, `PLUGIN-001`, and `QUAL-001` through
  `QUAL-002`; and `QUAL-002` as the first default-ready item.

- [ ] **Step 1: Verify the canonical map does not already exist**
...
Expected: exit code `1` with no output, proving this task is creating the
canonical file rather than overwriting an existing user-maintained map.

- [ ] **Step 2: Create the canonical map with the exact initial content**

Use `apply_patch` to create `docs/project-map.md` with:
...
- [ ] **Step 3: Validate the map structure, IDs, state, and dependencies**
...
const text = await readFile('docs/project-map.md', 'utf8')
...
const ids = [
  ...text.matchAll(
    /^### ((?:CORE|THEME|AI|PLUGIN|QUAL)-[0-9]{3}) —/gm,
  ),
].map((match) => match[1])
const expected = [
  'QUAL-002',
...
if (JSON.stringify(ids) !== JSON.stringify(expected)) {
  throw new Error('Unexpected item order: ' + ids.join(', '))
}
if (new Set(ids).size !== ids.length) throw new Error('Duplicate item ID')

for (const match of text.matchAll(/^- Depends on: (.+)$/gm)) {
  for (const dependency of match[1].matchAll(
    /`((?:CORE|THEME|AI|PLUGIN|QUAL)-[0-9]{3})`/g,
  )) {
    if (!ids.includes(dependency[1])) {
      throw new Error('Unknown dependency: ' + dependency[1])
    }
  }
}

if (!text.includes('- Default next item: `QUAL-002`')) {
  throw new Error('Default next item is not QUAL-002')
}
console.log('project map structure OK')
NODE
```
...
```text
project map structure OK
```

- [ ] **Step 4: Run targeted Markdown formatting**
...
If Prettier reports only this new file, run
`pnpm exec prettier --write docs/project-map.md`, inspect the resulting diff,
and repeat the check.

- [ ] **Step 5: Commit the canonical map**

```sh
git add docs/project-map.md
git commit -m "docs: add canonical Silen project map"
```

Expected: one commit containing only `docs/project-map.md`.

### Task 2: Add the repository agent routing contract
...
- Consumes: the canonical map path and its lifecycle vocabulary from Task 1.
- Produces: repository-wide default execution precedence for compatible agents
  without copying any map item ID, state table, or priority list.

- [ ] **Step 1: Verify the root agent entry point does not exist**
...
- [ ] **Step 2: Create the minimal routing contract**

Use `apply_patch` to create `AGENTS.md` with:
...
- Read [`docs/project-map.md`](docs/project-map.md) before proposing or
  starting roadmap work.
- Explicit user instructions override the map.
- With no special scope, safely continue the single `Active` item. If none is
  active, select the first `Ready` item whose dependencies are satisfied.
- `Candidate` and `Watch` items are not default-executable.
- Follow the repository's design and implementation-plan workflow before
  changing product behavior.
- When durable mapped work is completed, update the map state and verification
  evidence in the same change set.
- The map grants prioritization, not authority. Do not infer permission to
  publish, deploy, delete data, or change external systems.
```

- [ ] **Step 3: Verify discoverability without roadmap duplication**

Run:

```sh
test "$(grep -c 'docs/project-map.md' AGENTS.md)" -eq 1
if grep -Eq '(CORE|THEME|AI|PLUGIN|QUAL)-[0-9]{3}' AGENTS.md; then
  echo 'AGENTS.md duplicates project-map item state'
  exit 1
fi
grep -Fq 'Explicit user instructions override the map.' AGENTS.md
grep -Fq 'The map grants prioritization, not authority.' AGENTS.md
```
...
```sh
git add AGENTS.md
git commit -m "docs: route repository work through project map"
```

Expected: one commit containing only `AGENTS.md`.

### Task 3: Link the map and close the consistency gate

**Files:**

- Modify: `README.md`
- Verify: `AGENTS.md`
- Verify: `docs/project-map.md`
- Verify:
  `docs/superpowers/specs/2026-07-29-silen-project-map-design.md`

**Interfaces:**

- Consumes: the canonical map and agent entry point from Tasks 1 and 2.
- Produces: a human-facing README entry and final evidence that all local links,
  map invariants, and changed Markdown files are valid.

- [ ] **Step 1: Verify the README does not already expose the map**

Run:

```sh
grep -Fq '[project map](./docs/project-map.md)' README.md
```

Expected: exit code `1` with no output.

- [ ] **Step 2: Add the canonical map link under Contributing**

Use `apply_patch` to insert this paragraph immediately after the
`## Contributing` heading and before the existing contribution paragraph:
...
The resulting section must read:
...
- [ ] **Step 3: Verify every repository-relative Markdown link resolves**
...
const files = ['README.md', 'AGENTS.md', 'docs/project-map.md']
for (const file of files) {
  ...
  for (const match of text.matchAll(/\[[^\]]+\]\(([^)]+)\)/g)) {
    ...
    if (!existsSync(absolute)) {
      throw new Error(file + ' has a broken link: ' + target)
    }
  }
}
console.log('project map links OK')
NODE
```
...
```text
project map links OK
```

- [ ] **Step 4: Re-run structural and authority-boundary checks**
...
if (ids.length !== new Set(ids).size) throw new Error('Duplicate item ID')
if ((map.match(/^## Active$/gm) ?? []).length !== 1) {
  throw new Error('Map must have exactly one Active section')
}
if (!map.includes('No map-selected item is active.')) {
  throw new Error('Initial Active state is ambiguous')
}
if (!map.includes('- Default next item: `QUAL-002`')) {
  throw new Error('Unexpected default next item')
}
if (/(CORE|THEME|AI|PLUGIN|QUAL)-[0-9]{3}/.test(agents)) {
  throw new Error('AGENTS.md duplicates item state')
}
if (!readme.includes('[project map](./docs/project-map.md)')) {
  throw new Error('README map link is missing')
}
console.log('project map consistency OK')
NODE
```
...
pnpm exec prettier --check \
  ...
  docs/project-map.md \
  docs/superpowers/specs/2026-07-29-silen-project-map-design.md \
  docs/superpowers/plans/2026-07-29-silen-project-map.md
git diff --check
git status --short
```
...
- Before the final commit, `git status --short` lists only the intended README
  modification.

- [ ] **Step 6: Commit the README entry**

```sh
git add README.md
git commit -m "docs: expose Silen project map"
```

Expected: one commit containing only `README.md`.

- [ ] **Step 7: Verify the completed implementation**
...
- The three newest implementation commits are the canonical map, agent routing
  contract, and README entry.
- The project map identifies `QUAL-002` as the first default-ready item.

Do not push, publish, deploy, or begin `QUAL-002` as part of this plan unless
the user explicitly expands the current scope.
